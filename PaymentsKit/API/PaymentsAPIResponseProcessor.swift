...
{
    typealias PaymentsAPIError = APIError<PaymentsAPIErrorModel>
...
extension PaymentsAPIResponseProcessor: ResponseProcessor
{
                                           ...
                                           decoder: JSONDecoder?) -> Result<ResponseModel, Error>
    {
        ...
        if let error: PaymentsAPIError = errorHandler.error(request: request,
        ...
        // Null data error
        if data == nil { return .failure(PaymentsAPIError.emptyData) }
        ...
        catch let error
        {
            ...
            logger.error(.networking, domain: .paymentsKit)
            {
                guard let data = data,
            ...
            return .failure(PaymentsAPIError.decoding(error))
        }
    }
}
