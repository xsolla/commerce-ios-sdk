...
class StoreAPIResponseProcessor
{
    typealias StoreAPIError = APIError<StoreAPIErrorModel>
...
extension StoreAPIResponseProcessor: ResponseProcessor
{
                                           ...
                                           decoder: JSONDecoder?) -> Result<ResponseModel, Error>
    {
        ...
        if let error: StoreAPIError = errorHandler.error(request: request,
        ...
        // Null data error
        if data == nil { return .failure(StoreAPIError.emptyData) }
        ...
        catch let error
        {
            ...
            logger.error(.networking, domain: .storeKit)
            {
                guard let data = data,
            ...
            return .failure(StoreAPIError.decoding(error))
        }
    }
}
