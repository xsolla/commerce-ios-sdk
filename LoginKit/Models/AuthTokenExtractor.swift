...
public class AuthTokenExtractor: AuthTokenExtracting
{
    public func extract(from url: URL) -> Result<String, Error>
    {
        guard
            let urlComponents = URLComponents(url: url, resolvingAgainstBaseURL: false),
            let code = urlComponents.queryItems?.first(where: { $0.name == "token" })?.value
        else
            { return .failure(extractError(url: url) ?? LoginKitError.authTokenExtractionError("Unknown error")) }
    ...
    public func extract(from result: Result<URL, Error>) -> Result<String, Error>
    {
        ...
        {
            return .failure(LoginKitError.authTokenExtractionError("Unknown error"))
        }
    ...
    public func extract(from result: Result<String, Error>) -> Result<String, Error>
    {
        ...
        if case .success(let urlString) = result
        {
            ...
            else { return .failure(LoginKitError.authTokenExtractionError("Invalid url string")) }
        }
